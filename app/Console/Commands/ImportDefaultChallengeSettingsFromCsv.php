...
class ImportDefaultChallengeSettingsFromCsv extends Command
{
  /**
   ...
   * @var string
   */
  protected $signature = 'default-challenge-settings:import {file=new_default_challenge_settings.csv} {--chunk=1000}';
   ...
   * @var string
   */
  protected $description = 'Import default challenge settings from CSV file';
  ...
  public function handle()
  {
    ...
    // Load existing user UUIDs for validation
    $this->info("Loading existing user UUIDs for validation...");
    $userUuids = DB::table('users')->pluck('uuid')->toArray();
    $this->info("Loaded " . count($userUuids) . " user UUIDs");
    ...
    while (($row = fgetcsv($file)) !== false) {
      ...
      // Prepare default challenge settings data
      $settingsData = $this->prepareDefaultChallengeSettingsData($data);
  ...
  /**
   * Prepare default challenge settings data from CSV row
   */
  private function prepareDefaultChallengeSettingsData(array $data): array|false|null
  {
    try {
      return [
  ...
  /**
   * Insert batch of default challenge settings
   */
  private function insertBatch(array $batch, int &$successCount, int &$errorCount): void
  {
    try {
      DB::table('default_challenge_settings')->insert($batch);
      $successCount += count($batch);
    } catch (\Exception $e) {
      // Try inserting one by one to identify problematic rows
      foreach ($batch as $row) {
        try {
          DB::table('default_challenge_settings')->insert($row);
          $successCount++;
